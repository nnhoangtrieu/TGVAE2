...
class EncoderLayer(nn.Module) : 
    def __init__(self, d_model, n_heads, feed_forward, dropout) : 
        ...
        self.attn = gnn.PointTransformerConv(d_model, d_model, dropout = dropout)
    ...
    def forward(self, nf, coor, ei) : 
        nf = nf + self.drop1(self.attn(self.norm1(nf), coor, ei).relu())
...
class Encoder(nn.Module) : 
    ...
    def forward(self, nf, coor, ei, batch) : 
...
class Transformer(nn.Module):
    def __init__(self, d_model, d_latent, d_ff, e_heads, d_heads, num_layer, dropout, vocab, gvocab) : 
        ...
        self.src_embedding = gnn.PointTransformerConv(1, d_model)
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):  
        nf, ei, coor, batch = src.x, src.edge_index, src.coor, src.batch
        nf = nf.unsqueeze(-1).float()
        nf = self.src_embedding(nf, coor, ei)
        nf = self.norm(nf).relu()

        tgt = self.tgt_embedding(tgt)
        z, mu, sigma = self.encoder(nf, coor, ei, batch)
