...
class GraphPositionwiseFeedFoward(nn.Module) : 
    def __init__(self, d_model, d_ff, dropout=0.1) : 
        super(GraphPositionwiseFeedFoward, self).__init__()
        self.w_1 = gnn.GCNConv(d_model, d_ff)
        self.w_2 = gnn.GCNConv(d_ff, d_model)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x, ei) : 
        return self.w_2(self.dropout(F.relu(self.w_1(x, ei))), ei)
...
class EncoderLayer(nn.Module) : 
    ...
    def forward(self, nf, ei) : 
        ...
        nf = nf + self.drop2(self.feed_forward(self.norm2(nf), ei))
        return nf 
...
class Transformer(nn.Module):
    def __init__(self, d_model, d_latent, d_ff, e_heads, d_heads, num_layer, dropout, vocab, gvocab) : 
        ...
        gff = GraphPositionwiseFeedFoward(d_model, d_ff)
        position = PositionalEncoding(d_model, dropout)

        self.encoder = Encoder(EncoderLayer(d_model, e_heads, c(gff), dropout), num_layer, d_latent)
